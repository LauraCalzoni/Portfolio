...
# Course project number 2: Aircraft
# Students of group 15:
...
phy_limit = False   
step = 1        # flag to change according to the step to show
...
for kk in range(max_iters-1):
    ...
    if 0:
        steps = np.linspace(0,stepsize,int(1e1))
        costs = np.zeros(len(steps))
        for j in range(len(steps)):
            for tt in range(0,TT):
                uu_tmp[:,tt] = uu[:,tt,kk] + delta_uu[:,tt]*steps[j]
            costs[j] = cst.full_cost(xx_init, uu_tmp, xx_ref, uu_ref,phy_limit,kk,epsylon)[0]  
...
# Plots
############################
ut.plot_graph(uu_star[0],uu_ref[0],"T","u0")
ut.plot_graph(uu_star[1],uu_ref[1],"M","u1")
ut.plot_graph(xx_star[2],xx_ref[2],"theta(t)",2)
ut.plot_graph(xx_star[1],xx_ref[1],"z(t)",1)
ut.plot_graph(xx_star[0],xx_ref[0],"x(t)",0)
ut.plot_graph(xx_star[4],xx_ref[4],"gamma(t)",4)
ut.plot_graph(xx_star[3],xx_ref[3],"v(t)",3)
ut.plot_graph(xx_star[5],xx_ref[5],"q(t)",5)
ut.plot_graph(JJ[:max_iters-1],None,"JJ",)
ut.plot_graph(np.abs(descent[:max_iters-1]),None,"descent",)
plt.show(block=True)
...
if(step==3):
    ...
    # Plots
    ############################
    ut.plot_graph(uu_track[0],uu_star[0],"T","u0")
    ut.plot_graph(uu_track[1],uu_star[1],"M","u1")
    ut.plot_graph(xx_track[1],xx_star[1],"z(t)",1)
    ut.plot_graph(xx_track[0],xx_star[0],"x(t)",0)
    plt.show(block=True)
