...
# collection of data, binarization of the dataset
# reduction of the dataset size, downsampling, normalization
...
for agent in range(NN):
    ...
    for img in range(TRAIN_AGENT_DATA):            
        ...
        # loss_grad[0] --> lambdaT
        _, grad = backward_pass(xx, uu[agent], loss_grad) #adjoint method inside
...
for epoch in range(N_EPOCHS):
    for agent in range(NN):
        ...
        for img in range(TRAIN_AGENT_DATA):
            ...
            # Loss evalutation
            loss_grad = np.zeros((d))
            loss, loss_grad[0] = binary_cross_entropy(xx[-1, 0], y_train[agent, img]) #lambdaT 
            #[0] due to the mask of the d-1 last nodes

            # Backward pass (adjoint dynamics)
            _, grad = backward_pass(xx, uu_kp1[agent], loss_grad) #returning delta_u^i,k for each image
