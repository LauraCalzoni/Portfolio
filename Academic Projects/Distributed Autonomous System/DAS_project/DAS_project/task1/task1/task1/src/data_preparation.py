...
def split_images_per_agents(images, labels, n_agents, imgs_per_agent):
    ...
    #each row corresponds to an agent, the elements of that row are the indexes of the corresponing images
    ...
    agents_idxs = np.concatenate((idx_ones, idx_zeros), axis=1) #adding the other part of the agent's dataset
    ...
    for idxs in agents_idxs:
        ...
        #the images associated to different agents are placed one after the other
